...
담당자가 게시하면 팀원 전원이 같은 숫자를 열람하고, 월간 보고용 표·차트·분석 의견까지 한 화면에서 정리해 인쇄하거나 파일로 보관.
...
![무엇이든 찾기](docs/guide-nlq.png)

| | 무엇 |
|---|---|
| **1** | **찾기 버튼** — 어느 화면에서든 오른쪽 아래 같은 자리 |
| **2** | **묻는 칸** — 현장·공종·기간·내용을 섞어서 그대로 입력 · 오른쪽 `✕`로 지움 |
| **3** | **나뉜 조건** — 무엇으로 찾았는지 표시 · 사전에 없는 말은 접수내용에서 함께 찾음 |
| **4** | **목록 창으로** — 같은 조건이 걸린 목록이 열려 정렬·피벗·엑셀 그대로 사용 |

- 현장 이름은 **일부만 쳐도** 찾힘 — `힐스테이트 두정역` = `두정역` = `두정`
  - 여러 현장이 함께 쓰는 말(`힐스테이트`)은 어느 현장인지 못 정하므로 더 적어 달라고 안내
- 사전에 없는 말은 **본문 검색어**로 씀 — 접수내용·하자유형·공간·업체·동호까지 훑음
  - 예: `타일 누수` = 공종 타일 + 접수내용에 '누수' · 여러 낱말은 모두 포함된 건만
- **미처리 목록에서만** 찾음 — 처리완료 건은 대상 아님
- 빈 칸일 때는 **최근 찾은 것**이 뜸 — 눌러 다시 찾기 · 하나씩 또는 모두 지우기
  - 이 PC의 브라우저에만 저장(팀 공유 아님) · 결과가 있는 검색만 기록
- 외부 서비스를 쓰지 않음 — 조건은 규칙이 만들고 **숫자는 앱이 셈**
  - 화면의 다른 숫자와 항상 일치 · 인터넷 없이 열리는 스냅샷에서도 동작

### 하자처리현황 — 묶는 기준 바꾸기

- 대시보드와 현장 화면의 표 오른쪽 위 **[현장별|업체별]** · **[공종별|업체별]** 로 전환
- **제목이 기준을 따라 바뀜** — 인쇄물만 봐도 무슨 기준인지 알 수 있음
- 바뀌는 것은 앞쪽 설명 열뿐 — 숫자 열과 **합계는 그대로**
- 업체는 엑셀의 **시공업체** 컬럼 기준 · 비어 있는 건은 `(미기재)` 행으로 따로 집계
- 표는 **상위 15개 + 나머지 묶음** · 합계는 언제나 전체 기준
  - 현장 화면에서는 **「그 외 N개」 행을 누르면 펼쳐짐**(다시 접기도 행 클릭)
- 기본은 **대시보드=현장별 · 현장 화면=공종별** · 바꾼 선택은 이 PC의 브라우저에 기억됨
- **인쇄는 화면에서 고른 기준 그대로** 나감(세그먼트 버튼·접기 행은 인쇄에서 제외)
- 업체별 자료는 **재게시한 달부터** 팀원에게 보임(옛 게시본은 안내 문구 표시)
...
- **처리계획** — 현장 > 장기미처리 탭의 공종별 입력란 · 열람자도 작성 가능
  - 한 칸 안에서 **위=지난달 계획(읽기) · 아래=이번 달 계획(입력)** — 그 옆 전월→금월 숫자가 곧 이행 결과
  - 장기미처리·공가세대·상가 세 탭 모두 같은 형태
  - 지난달 계획이 없으면 `-`
