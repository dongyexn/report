...
- 화면 **오른쪽 아래 파란 버튼** — 어느 화면에서든 같은 자리
- 현장·공종·기간을 섞어서 입력 — 예: `신용 도배 60일 넘은 거` · `두정 공가세대` · `익산 101동`
- 알아들은 조건은 칩으로 표시 · 못 알아들은 말은 그대로 알려줌
- **미처리 목록에서만** 찾음(처리완료 건은 대상 아님)
- 「이 조건으로 목록 창 열기」 → 정렬·피벗·엑셀 내보내기 그대로 사용
- 외부 서비스를 쓰지 않음 — 조건은 규칙이 만들고 숫자는 앱이 셈 · 스냅샷에서도 동작
