...
function siteAxParts(sid,st){
  ...
  if(!S.axSiteAll && named.length>SITE_TOP_N){        // 상위 10개 + 나머지 한 줄(행을 누르면 펼침)
...
function vacRowsHTML(sid,stat,cm,planField){
  ...
  const totFn=(tot,prevTot)=>{
    ...
    return`<tr class="tot"><td class="cc"></td><td><b>합계</b></td><td></td><td class="n"><b>${prevTot.toLocaleString()}</b></td><td class="n" style="color:var(--bt1)"><b>${tot.c.toLocaleString()}</b></td><td class="cc" style="white-space:nowrap"><span class="ba ${dBadge}" data-tt="전월 ${prevTot.toLocaleString()} → 금월 ${tot.c.toLocaleString()}" aria-label="전월 ${prevTot.toLocaleString()} → 금월 ${tot.c.toLocaleString()}">${dArrow} ${dTxt}</span></td><td class="cc"><b>100.0%</b></td><td class="pp-cell"><div style="min-height:32px"></div></td></tr>`;
  };
