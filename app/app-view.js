// ─────────────────────────────────────────────────────────────────────────────
// app-view.js — 화면 렌더: 내비게이션·대시보드·현장 패널·차트.
...
// 대시보드 업체별 행 — 현장별 행과 '똑같은 마크업'을 쓴다(비율 칸의 ltrbar-wrap·건수 기준 증감).
//   구조가 다르면 폭이 같아도 줄바꿈·정렬이 어긋난다(실제로 그랬음).
function dashCoRowHTML(x,i,opt){
  opt=opt||{};
  const rate=x.r>0?(x.res/x.r*100):0, ltr=x.u>0?(x.lt/x.u*100):0;
  ...
  const ld=x.lt-x.plt,arrow=ld===0?'─':ld>0?'▲':'▼',sign=ld===0?'':ld>0?'+':'−',badge=ld===0?'bgr':ld>0?'brd':'bgn';
  ...
  return `<tr${opt.tot?' class="tot"':''}>`+
    ...
    `<td class="cc" style="white-space:nowrap"><span class="ba ${uBadge}" data-tt="전월 ${x.pu.toLocaleString()} → 금월 ${x.u.toLocaleString()}">${uArrow} ${uSign}${Math.abs(uD).toLocaleString()}</span></td>`+
...
function siteAxParts(sid,st){
  ...
  if(!S.axSiteAll && named.length>SITE_TOP_N){        // 상위 N개 + 나머지 한 줄(행을 누르면 펼침)
    const rest=named.slice(SITE_TOP_N);
    ...
    list=[...named.slice(0,SITE_TOP_N),f,...na]; folded=true;
  }
  const rows=list.map((x,i)=>aggRowHTML(x,i,ax,sid,false,!!(x.fold||x.key==='(미기재)'),false,!!x.fold)).join('')
    +(!folded&&named.length>SITE_TOP_N?`<tr class="axfold" data-act="axis.siteAll"><td colspan="11">접기 — 상위 ${SITE_TOP_N}${unit}만 보기</td></tr>`:'');
...
function rDashAxis(list){
  ...
  const tfoot = ordered.length ? `<tfoot>${dashCoRowHTML(Object.assign({},T,{key:'합계',side:'',sideN:0}),0,{tot:true})}</tfoot>` : '';
...
function aggRowHTML(x,i,axis,sid,noIdx,blankIdx,isTot,foldRow){
...
function go(view,sid){
  ...
  if(view==='manage')fireHook('view.manage');
  if(view==='settings')fireHook('view.settings');
...
// 계획 칸은 한 열 안에서 위=지난달(읽기) · 아래=이번 달(입력) 두 단으로 쌓는다.
...
function vacRowsHTML(sid,stat,cm,planField){
  ...
  const rowFn=(t,i)=>{
    ...
    return`<tr><td class="cc"><b>${i+1}</b></td><td class="rl-link" data-act="rec.list" data-sid="${esc(sid)}" data-scope="ul" data-trade="${esc(t.t)}" data-vac="${_vk}"><b>${esc(t.t)}</b></td><td>${coCell}</td><td class="n">${pc.toLocaleString()}</td><td class="n" style="color:var(--bt1);font-weight:700">${t.c.toLocaleString()}</td><td class="cc" style="font-weight:600">${ratio}%</td><td class="cc" style="white-space:nowrap"><span class="ba ${dBadge}" data-tt="전월 ${pc.toLocaleString()} → 금월 ${t.c.toLocaleString()}" aria-label="전월 ${pc.toLocaleString()} → 금월 ${t.c.toLocaleString()}">${dArrow} ${dTxt}</span></td><td class="pp-cell">${prevPlanTop(cm[planField],t.t)}<div class="pp-stack"><span class="pp-lab">이번 달</span><textarea class="inp plan-ta" maxlength="5000" aria-label="처리계획" name="plan-${sid}-${planField}-${esc(t.t)}" data-plan-id="cmt|${sid}|${planField}|${esc(S.rm+'@'+t.t)}" style="padding:5px 9px;font-size:11.5px;min-height:32px;resize:none;font-family:inherit;width:100%;overflow:hidden" placeholder="" data-act="panel.plan">${esc(plan)}</textarea></div></td></tr>`;
  };
  const etcFn=(etc)=>{
    if(!etc)return'';
...
function rSite(sid){
  ...
  const trRowFn=(t,i,planMap,planKey)=>{
    ...
    const {dArrow,dBadge,dTxt}=deltaParts(dN);
    const coCell=esc(t.co||'-');
    return`<tr><td class="cc"><b>${i+1}</b></td><td class="rl-link" data-act="rec.list" data-sid="${esc(sid)}" data-scope="lul" data-trade="${esc(t.t)}"><b>${esc(t.t)}</b></td><td>${coCell}</td><td class="n">${pc.toLocaleString()}</td><td class="n" style="color:var(--bt1);font-weight:700">${t.c.toLocaleString()}</td><td class="cc" style="font-weight:600">${ratio}%</td><td class="cc" style="white-space:nowrap"><span class="ba ${dBadge}" data-tt="전월 ${pc.toLocaleString()} → 금월 ${t.c.toLocaleString()}" aria-label="전월 ${pc.toLocaleString()} → 금월 ${t.c.toLocaleString()}">${dArrow} ${dTxt}</span></td><td class="pp-cell">${prevPlanTop(planMap,t.t)}<div class="pp-stack"><span class="pp-lab">이번 달</span><textarea class="inp plan-ta" maxlength="5000" aria-label="처리계획" name="plan-${sid}-${planKey}-${esc(t.t)}" data-plan-id="cmt|${sid}|${planKey==='pp'?'processingPlan':'vacantProcessingPlan'}|${esc(S.rm+'@'+t.t)}" style="padding:5px 9px;font-size:11.5px;min-height:32px;resize:none;font-family:inherit;width:100%;overflow:hidden" placeholder="" data-act="panel.plan">${esc(plan)}</textarea></div></td></tr>`;
  };
  ...
  const trEtcFn=(etc,prevMap)=>{
    ...
    return`<tr data-fixed="1"><td class="cc"></td><td><b>기타</b></td><td>${coCell}</td><td class="n">${pc.toLocaleString()}</td><td class="n" style="color:var(--bt1);font-weight:700">${etc.c.toLocaleString()}</td><td class="cc" style="font-weight:600">${ratio}%</td><td class="cc" style="white-space:nowrap"><span class="ba ${dBadge}" data-tt="전월 ${pc.toLocaleString()} → 금월 ${etc.c.toLocaleString()}" aria-label="전월 ${pc.toLocaleString()} → 금월 ${etc.c.toLocaleString()}">${dArrow} ${dTxt}</span></td><td class="pp-cell">${prevPlanTop(cm.processingPlan,'기타')}<div class="pp-stack"><span class="pp-lab">이번 달</span><textarea class="inp plan-ta" maxlength="5000" aria-label="처리계획" name="plan-${sid}-pp-기타" data-plan-id="cmt|${sid}|processingPlan|${esc(S.rm+'@기타')}" style="padding:5px 9px;font-size:11.5px;min-height:32px;resize:none;font-family:inherit;width:100%;overflow:hidden" placeholder="" data-act="panel.plan">${esc(planEtc)}</textarea></div></td></tr>`;
  };
  // 합계 행
...
<div class="tpane ${S.tab==='overview'?'act':''}" data-tab="overview">
  <div class="as">
    <div class="card main-chart-card" data-print="ov-chart"><div class="sh" style="margin-bottom:6px;flex-shrink:0"><div class="ct cardttl">하자접수 · 처리 주차별 추이</div><select class="yr-sel no-print" id="strend-yr" data-act="site.trendYear" aria-label="현장 추이 연도 선택"></select></div><div class="cw" style="flex:1;min-height:0"><canvas id="c-mo-${sid}"></canvas></div><div id="c-mo-lg-${sid}" class="chart-lg" style="padding-left:48px;flex-shrink:0"><div class="li"><span class="mk-bar ck-d60"></span>60일 이상</div><div class="li"><span class="mk-bar ck-d30"></span>30~59일</div><div class="li"><span class="mk-bar ck-d0"></span>30일 미만</div><div class="li"><span class="mk-ln ck-recv"></span>전체 접수</div><div class="li"><span class="mk-ln ck-done"></span>처리 완료</div></div></div>
